...
const Menu = {
  async getAllMenuCategoryByCanteen(id) {
    ...
    for (const row of rows) {
      ...
      let menu = menuCategory.menus.find((m) => m.menu_id === row.menu_id);
      if (!menu) {
        menu = {
          menu_id: row.menu_id,
          ...
          menu_is_available: row.menu_is_available,
        };
  ...
  async getMenuById(id) {
    ...
    for (const row of rows) {
      ...
      let addOn = addOnCategory.addOns.find((a) => a.addon_id === row.addon_id);
      if (!addOn) {
        addOn = {
        ...
        addOnCategory.addOns.push(addOn);
      }
    }
    console.log(rows)
    ...
    const menu = {
      ...
      menu_is_available: rows[0].menu_is_available,
